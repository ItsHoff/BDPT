...
class AreaLight {
 public:
    ...
    // Draw light to the openGl viewport
    void draw(const Mat4f& worldToCamera, const Mat4f& projection);
    // Draw a sample point from the light source.
    void sample(F32& pdf, Vec3f& p, Random& rnd);

    Vec3f getPosition(void) const {
        return Vec4f(m_xform.getCol(3)).getXYZ();
    }
    void setPosition(const Vec3f& p) {
        m_xform.setCol(3, Vec4f(p, 1.0f));
    }

    Mat3f getOrientation(void) const {
        return m_xform.getXYZ();
    }
    ...
    Vec3f getNormal(void) const {
        return -Vec4f(m_xform.getCol(2)).getXYZ();
    }
